...
# ==============================================================================
#  ArttulOS Automated INSTALLER ISO Builder v24.0 (Correct UEFI Fix)
#
#  - MY SINCERE APOLOGIES. You were 100% correct. The previous script did
#    not modify the UEFI boot configuration properly.
#  - THE FIX: The script now correctly modifies `EFI/BOOT/BOOT.conf` for
#    UEFI systems, which is the proper file for boot menu entries. It appends
#    the kickstart parameter to the 'options=' line. This is the correct method.
#
...
# Generated by ArttulOS Builder v24.0 - Installer Kickstart
graphical
lang en_US.UTF-8
...
main() {
    ...
    echo -e "${BLUE}======================================================================${NC}"
    echo -e "${BLUE}  ArttulOS Automated INSTALLER ISO Builder v24.0                      ${NC}"
    echo -e "${BLUE}======================================================================${NC}"
    ...
    ( # --- Start Subshell for Safe Operations ---
        ...
        local KS_PARAM=" inst.ks=hd:LABEL=${ISO_LABEL}:/${KS_FILENAME}" # Note the leading space
        ...
        # 2. For UEFI systems (BOOT.conf) - This is the critical fix.
        sed -i "/^options=/ s@\$@${KS_PARAM}@" "iso_root/EFI/BOOT/BOOT.conf"
        sed -i "s/Rocky Linux/${DISTRO_NAME}/g" "iso_root/EFI/BOOT/BOOT.conf"
        # Also brand the main grub config file's title for consistency
