...
# ==============================================================================
#  ArttulOS Automated INSTALLER ISO Builder v29.0 (The Definitive Fix)
#
#  - MY SINCERE APOLOGIES. This is a complete rewrite of the %post section
#    to finally fix the kernel loading issue.
#  - THE FIX:
#    1. Forcefully rebuilds the initramfs for kernel-ml, ensuring LVM drivers
#       are included so the root filesystem can be found.
#    2. Rebuilds grub.cfg from scratch.
#    3. Surgically corrects the kernel paths in grub.cfg.
#  - This is the robust, correct procedure that should have been used from the start.
#
...
# Generated by ArttulOS Builder v29.0 - Installer Kickstart
graphical
lang en_US.UTF-8
...
# The custom kernel and its dependencies
kernel-ml
-kernel
elrepo-release
gnome-initial-setup
# Add dracut-config-generic for rebuilding initramfs
...
# =====================================================================
# === KERNEL BOOT FIX - THIS IS THE CRITICAL PART ===
# =====================================================================
echo "Step 1: Forcing a rebuild of the initramfs for the new kernel"
ML_KERNEL_VERSION=\$(ls /boot/vmlinuz-*.elrepo.x86_64 | head -n 1 | sed 's/^\/boot\/vmlinuz-//')
if [ -z "\$ML_KERNEL_VERSION" ]; then
    echo "FATAL: Could not find kernel-ml to rebuild initramfs." >&2
    exit 1
fi
# Forcefully rebuild, explicitly adding lvm support.
dracut --force --add lvm /boot/initramfs-\${ML_KERNEL_VERSION}.img \${ML_KERNEL_VERSION}
echo "Initramfs rebuilt for \${ML_KERNEL_VERSION}"

echo "Step 2: Rebuilding GRUB configuration from scratch"
GRUB_CFG_PATH=""
# Find the correct grub.cfg path (UEFI first, then BIOS)
...
echo "GRUB config rebuilt at \${GRUB_CFG_PATH}"

echo "Step 3: Correcting kernel paths for LVM installations"
# This is a known issue when running grub2-mkconfig in a chroot.
sed -i 's#\(^\s*\)\(linux\|initrd\) /boot/#\2 /#' "\${GRUB_CFG_PATH}"
echo "Corrected paths in grub.cfg"

echo "ArttulOS Post-Installation Complete. System should now boot kernel-ml."
%end
EOF
...
main() {
    ...
    echo -e "${BLUE}======================================================================${NC}"
    echo -e "${BLUE}  ArttulOS Automated INSTALLER ISO Builder v29.0                      ${NC}"
    echo -e "${BLUE}======================================================================${NC}"
    ...
    ( # --- Start Subshell for Safe Operations ---
        ...
        sed -i "/menu label .*Install ${DISTRO_NAME}/,/^  append/s@^\(  append .*\)$@\1${KS_PARAM}@" "iso_root/isolinux/isolinux.cfg"
        sed -i "/menuentry 'Install ${DISTRO_NAME}/,/^  linuxefi/s@^\(  linuxefi .*\)$@\1${KS_PARAM}@" "iso_root/EFI/BOOT/grub.cfg"
        echo -e "${GREEN}    Bootloaders configured correctly.${NC}"
