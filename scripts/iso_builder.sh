...
timezone ${KS_TIMEZONE} --isUtc; network --onboot=yes --device=eth0 --bootproto=dhcp --ipv6=auto --activate
network --hostname=${KS_HOSTNAME}; firewall --enabled --service=ssh; selinux --enforcing
zerombr; clearpart --all --initlabel; autopart --type=lvm; bootloader --location=mbr
rootpw --plaintext ${KS_PASS}; user --name=${KS_USER} --groups=wheel --password=${KS_PASS} --plaintext
...
@core; @fonts; @gnome-desktop; @guest-desktop-agents
kernel-ml; kernel-ml-devel
policycoreutils-python-utils; vim-enhanced; kexec-tools; plymouth-scripts; flatpak; curl
-rocky-logos; -rocky-logos-httpd; -rocky-logos-epel
%end
...
[org/gnome/desktop/background]; picture-uri='file://\${WALLPAPER_PATH}'; picture-uri-dark='file://\${WALLPAPER_PATH}'
[org/gnome/desktop/screensaver]; picture-uri='file://\${WALLPAPER_PATH}'
GSETTINGS_EOF
...
[Unit]; Description=ArttulOS First-Boot Online Application Installer; After=network-online.target; Wants=network-online.target
[Service]; Type=oneshot; ExecStart=/usr/local/sbin/arttulos-first-boot-setup.sh; ExecStartPost=/bin/rm -f /usr/local/sbin/arttulos-first-boot-setup.sh; ExecStartPost=/bin/systemctl disable arttulos-first-boot.service
...
patch_bootloader() {
    print_msg "blue" "Patching bootloader configs for automated installation..."; local isolinux_cfg="${ISO_EXTRACT_DIR}/isolinux/isolinux.cfg"; local grub_cfg="${ISO_EXTRACT_DIR}/EFI/BOOT/grub.cfg"
    local ks_append="inst.stage2=hd:LABEL=${ISO_LABEL} quiet inst.ks=hd:LABEL=${ISO_LABEL}:/ks.cfg inst.gtk.theme=Adwaita-dark"
    cat << EOF > "${isolinux_cfg}"; default vesamenu.c32; timeout 10; menu title ArttulOS 9 Installer; label install; menu label ^Install ArttulOS; menu default; kernel vmlinuz; append initrd=initrd.img ${ks_append}; EOF
    cat << EOF > "${grub_cfg}"; set timeout=1; menuentry 'Install ArttulOS' --class gnu-linux --class gnu --class os {; linuxefi /images/pxeboot/vmlinuz ${ks_append}; initrdefi /images/pxeboot/initrd.img; }; EOF
}
build_final_iso() {
