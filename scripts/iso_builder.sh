...
# ==============================================================================
#  ArttulOS Automated INSTALLER ISO Builder v35.0 (Definitive Bootloader Fix)
#
#  - MY SINCERE APOLOGIES. You are 100% correct. This script is now rewritten
#    based on the correct bootloader information for Rocky 9.
#  - THE FIX: The script no longer mounts efiboot.img. It correctly modifies
#    the single `/boot/grub2/grub.cfg` file on the ISO for both BIOS and UEFI
#    kickstart automation. This is the correct procedure.
#
...
# Generated by ArttulOS Builder v35.0 - Installer Kickstart
graphical
lang en_US.UTF-8
...
main() {
    ...
    echo -e "${BLUE}======================================================================${NC}"
    echo -e "${BLUE}  ArttulOS Automated INSTALLER ISO Builder v35.0                      ${NC}"
    echo -e "${BLUE}======================================================================${NC}"
    ...
    ( # --- Start Subshell for Safe Operations ---
        ...
        print_step "Configuring Bootloaders and Branding..."
        cp "$KS_FILENAME" "iso_root/"
        local KS_PARAM=" inst.ks=hd:LABEL=${ISO_LABEL}:/${KS_FILENAME}"

        # --- THE DEFINITIVE BOOTLOADER FIX ---
        # 1. Brand all bootloader configs
        sed -i "s@Rocky Linux@${DISTRO_NAME}@g" "iso_root/boot/grub2/grub.cfg"
        ...
        # 2. Add Kickstart parameter to the main installation entries
        sed -i 's@^\(.*\inst.stage2=.*\)@\1'"${KS_PARAM}"'@' "iso_root/boot/grub2/grub.cfg"
        sed -i 's@^\(.*\inst.stage2=.*\)@\1'"${KS_PARAM}"'@' "iso_root/isolinux/isolinux.cfg"
        echo -e "${GREEN}    Bootloaders configured correctly.${NC}"

        # 3. Apply visual branding to installer
        ...
        echo -e "${GREEN}    Installer image branded and repacked.${NC}"
    ...
    # Final cleanup and ownership change
    rm -rf "$BUILD_DIR"
    if [ -n "$SUDO_USER" ]; then
        chown "$SUDO_USER:$SUDO_USER" "$FINAL_ISO_PATH"
    else
        chown "$USER" "$FINAL_ISO_PATH"
    fi
