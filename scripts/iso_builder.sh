...
# ==============================================================================
#  ArttulOS Automated INSTALLER ISO Builder v23.0 (The Correct Method)
#
#  - MY SINCERE APOLOGIES. This is a complete rewrite to build the correct
#    ISO type. The previous scripts were for a LIVE ISO, which was wrong.
#  - This script correctly modifies a standard INSTALLER ISO, which is what
#    `virt-install` and physical installations require.
#  - It uses a simple, clean kickstart designed for installation, not live boot.
#  - It uses the robust `.discinfo` method for the Volume ID and subshells
#    for safe directory operations. This is the correct, final version.
#
...
generate_installer_kickstart() {
    ...
    # This is a simple, clean kickstart for a standard installation.
    # It does NOT include any of the complex livesys scripts.
    cat > "${kickstart_file_path}" <<EOF
# Generated by ArttulOS Builder v23.0 - Installer Kickstart
graphical
lang en_US.UTF-8
...
# Use the correct repositories for the installer
...
# The custom kernel
kernel-ml
# Remove the default kernel to avoid conflicts and ensure ml is default
-kernel
...
# Set DISTRO_NAME for use in branding
DISTRO_NAME="${DISTRO_NAME}"

# Branding
echo "Applying ArttulOS branding to final system..."
...
# Kernel Bootloader Fix - This is critical
echo "Configuring bootloader for kernel-ml..."
# Find the installed kernel path
ML_KERNEL_PATH=\$(ls /boot/vmlinuz-*.elrepo.x86_64 | head -n 1)
if [ -z "\$ML_KERNEL_PATH" ]; then
    echo "FATAL: Could not find kernel-ml in /boot." >&2
    exit 1
fi
...
# Rebuild the config file to register the new kernel
grub2-mkconfig -o "\$GRUB_CFG_PATH"

# Surgically fix paths for LVM installations (removes /boot/ prefix)
sed -i "s#\(^\s*\)\(linux\|initrd\) /boot/#\2 /#" "\$GRUB_CFG_PATH"

echo "ArttulOS Post-Installation Complete. Default kernel is now kernel-ml."
%end
EOF
...
main() {
    ...
    echo -e "${BLUE}======================================================================${NC}"
    echo -e "${BLUE}  ArttulOS Automated INSTALLER ISO Builder v23.0                      ${NC}"
    echo -e "${BLUE}======================================================================${NC}"
    ...
    ( # --- Start Subshell for Safe Operations ---
        ...
        print_step "Configuring Bootloader for Kickstart..."
